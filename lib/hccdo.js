...
// add bleed/safe to card size view
...
context.style = (function () {
	...
	function card(data) {
		...
		if (outerBorder) {
			style += overlay(cardSize,zeroSize,"Card",outerBorder,brSize);
		}
...
context.write = (function () {
	...
	function formatter(data,cards) {
		...
		if (data.overlay || data.border)
			templateB += '\n\t<div class="hccdOverlayCard"></div>';
		if (data.overlay)
			templateB += '<div class="hccdOverlayBleed"></div><div class="hccdOverlaySafe"></div>';
		templateB += "\n</div>{{/hccdo}}";
