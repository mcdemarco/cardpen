...
context.form = (function () {
	...
	function generate(e) {
		...
		context.init.generate(forImage);
	}
...
/*		var h, w;
		var pageSize = context.form.getRadio("pageSize");
		if (pageSize == "letter") {
			h = 215.9; //mm
			w = 279.4; //mm
		} else {
	...
	function columns() {
		//Get the number of columns for the current card/page size.
		return 4;
	}
...
context.write = (function () {
	...
	function generate(data,forImages) {
		...
		if (forImages) {
			...
			fullOutput += '    <script type="text/javascript">var dpi = ' + context.form.get("dpi") + ';\n var projectName = "' + context.form.get("projectName") + '";</script>\n';
		}
		fullOutput += "<style>\n" + data.css + "</style>\n</head>\n<body>\n";
		if (forImages) {
	...
	function formatter(cards,template) {
		...
		var rows = context.size.rows();
		var cols = context.size.columns();
		var formatted = '';
	...
	var cardForm = Bind({
		...
		'data.dpi': '#dpi',
		'data.extCSS': '#extCSS',
		'data.css': '#cssTextarea',
		'data.csv': '#csvTextarea',
		'data.mustache': '#mustacheTextarea'
	});
